...
class TossChart extends CustomPainter {
  ...
  TossChart(Size size,
      {required this.tossNTurnGraph, required this.tossNTurnGraphY}) {
  ...
  Picture _captureChart(Size size) {
    ...
    final topicPainter =
        TextPainter(text: topicSpan, textDirection: TextDirection.ltr);
    ...
    final minToss = TextSpan(
        text:
            ((int.parse(tossNTurnGraphY.minToss) / 10).floor() * 10).toString(),
    ...
    final minTossPainter =
        TextPainter(text: minToss, textDirection: TextDirection.ltr);

    minTossPainter.layout(maxWidth: 48);
    minTossPainter.paint(
        canvas,
        Offset(topicPainter.width / 2 - minTossPainter.width / 2,
            (size.height - 23) - minTossPainter.height / 2));

    final maxToss = TextSpan(
        text:
            ((int.parse(tossNTurnGraphY.maxToss) / 10).ceil() * 10).toString(),
        style: const TextStyle(
            fontSize: 12,
            fontWeight: FontWeight.w400,
            fontFamily: 'Pretendard',
    ...
    final maxTossPainter =
        TextPainter(text: maxToss, textDirection: TextDirection.ltr);

    maxTossPainter.layout(maxWidth: 48);
    maxTossPainter.paint(
        canvas,
        Offset(topicPainter.width / 2 - maxTossPainter.width / 2,
            32 - maxTossPainter.height / 2));

    canvas.drawLine(Offset(49, size.height - 23),
        Offset(size.width - 7, size.height - 23), xlines);
    ...
    for (var i = 0; i < tossNTurnGraph.length; i++) {
      ...
      xtextPainter.paint(canvas,
          Offset(startX + p / 2 - (xtextPainter.width / 2), xtextStartY));
    ...
    for (var i = 0; i < tossNTurnGraph.length; i++) {
      ...
      var y = ((size.height - 55) *
              (((maxT - minT) - (toss - minT)) / (maxT - minT))) +
          32;
      ...
      RRect rRect = RRect.fromRectAndCorners(rect,
          topLeft: Radius.circular(barWidth / 5),
          topRight: Radius.circular(barWidth / 5));
